...
class Message(dict):
    """ 
    The Message class is responsible for working with individual user posts. It currently supports two features: 
    A timestamp property that is set upon instantiation and when the entry object is set and an 
    entry property that stores the post message.
    """
    def __init__(self, message = None, From = None, timestamp:float = 0):
        self._timestamp = timestamp
        ...
        # Subclass dict to expose Post properties for serialization
        # Don't worry about this!
        dict.__init__(self, message=self._message, From = self._from, timestamp=self._timestamp)
    ...
    def set_from(self, From):
        self._from = From
        dict.__setitem__(self, 'from', From)
    ...
    timestamp = property(get_time, set_time)
    From = property(get_from, set_from)

class Conversation(dict):
    def __init__(self, messagelst): #after initializing Profile object, get_messages and pass the returned lst to form conversations
        self.username = 'codingcowgirls101'
        self.messages = messagelst
    
    def CreateConvo(self):
        '''Creates a conversation by recieving a list of messages from the post object.'''
        conversations = {}
        contacts = []
        for message in self.messages:
            recipient = message['recipient']
            messenger = message['messenger']
            if (recipient in contacts) and (messenger in contacts):
                pass
            elif (recipient in contacts) and (messenger not in contacts):
                contacts.append(messenger)
            elif (recipient not in contacts) and (messenger in contacts):
                contacts.append(recipient)
            else:
                contacts.append(recipient)
                contacts.append(messenger)
        if self.username in contacts:
            contacts.remove(self.username)

        for contact in contacts:
            convo_lst = []
            for message in self.messages:     
                if message['recipient']==contact:
                    convo_lst.append(message)
                elif message['messenger'] == contact:
                    convo_lst.append(message)
            conversations[contact]= convo_lst
